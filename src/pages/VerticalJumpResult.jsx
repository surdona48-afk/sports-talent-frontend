  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-8">
      <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 md:p-8 mb-8 shadow-xl text-center">
        <span className="bg-emerald-500/10 border border-emerald-500/30 text-emerald-400 text-xs font-bold uppercase tracking-wider px-3 py-1 rounded-full inline-block mb-3">
          ✓ Verified by AI Vision
        </span>
        <h1 className="text-3xl font-extrabold text-white mb-1">Vertical Jump Scorecard</h1>
        <p className="text-slate-400 text-sm">Assessment successfully recorded and synced.</p>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        <div className="bg-slate-900 border border-slate-800 rounded-xl p-6 text-center">
          <span className="block text-slate-500 text-xs uppercase font-semibold mb-1">Max Leap Height</span>
          <span className="text-4xl font-extrabold text-white">{result?.jumpHeight || '--'}</span>
        </div>

        <div className="bg-slate-900 border border-slate-800 rounded-xl p-6 text-center">
          <span className="block text-slate-500 text-xs uppercase font-semibold mb-1">Flight Hang Time</span>
          <span className="text-4xl font-extrabold text-white">{result?.hangTime || '--'}</span>
        </div>

        <div className="bg-slate-900 border border-slate-800 rounded-xl p-6 text-center">
          <span className="block text-slate-500 text-xs uppercase font-semibold mb-1">Overall AI Rating</span>
          <span className="text-4xl font-extrabold text-blue-400">{result?.score || '--'} / 100</span>
        </div>
      </div>
