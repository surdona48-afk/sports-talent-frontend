...
export default function ProfileMatch() {
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-8">
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 md:p-8 mb-8 shadow-xl flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
        <div>
          <div className="flex items-center gap-3 mb-3">
            <span className="bg-amber-500/10 border border-amber-500/30 text-amber-400 text-xs font-bold uppercase tracking-wider px-3 py-1 rounded-full">
      ...
      <div className="mb-10">
        <h2 className="text-xl font-bold text-white mb-4">Your Strongest Attributes</h2>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {strongestAttributes.map((attr, index) => (
            <div
              key={attr.id}
              className="bg-slate-900 border border-slate-800 rounded-2xl p-6 shadow-xl flex flex-col justify-between relative overflow-hidden group hover:border-blue-500/40 transition-all"
            >
              <div>
                <div className="flex justify-between items-center mb-4">
                  <span className="text-3xl bg-slate-950 p-3 rounded-xl border border-slate-800">
                    {attr.icon}
                  </span>
                ...
                <h3 className="text-xl font-extrabold text-white mt-0.5 mb-2">{attr.title}</h3>
                <p className="text-slate-400 text-sm leading-relaxed">{attr.description}</p>
              </div>
            </div>
          ))}
        </div>
