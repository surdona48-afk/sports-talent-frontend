  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-8">
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 md:p-8 mb-8 shadow-xl text-center">
        <span className="inline-flex items-center gap-2 bg-emerald-500/10 border border-emerald-500/30 text-emerald-400 text-xs font-bold uppercase tracking-wider px-4 py-1.5 rounded-full mb-3">
          <span className="w-2 h-2 rounded-full bg-emerald-400 animate-pulse"></span>
          🟢 Verified Assessment
        </span>
        <h1 className="text-3xl font-extrabold text-white mb-1">10m Sprint Scorecard</h1>
        <p className="text-slate-400 text-sm">Assessment successfully recorded, verified, and synced.</p>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        <div className="bg-slate-900 border border-slate-800 rounded-xl p-6 text-center">
          <span className="block text-slate-500 text-xs uppercase font-semibold mb-1">Sprint Time</span>
          <span className="text-4xl font-extrabold text-white">{result?.time || '--'}</span>
        </div>

        <div className="bg-slate-900 border border-slate-800 rounded-xl p-6 text-center">
          <span className="block text-slate-500 text-xs uppercase font-semibold mb-1">Top Velocity</span>
          <span className="text-4xl font-extrabold text-white">{result?.topSpeed || '--'}</span>
        </div>

        <div className="bg-slate-900 border border-slate-800 rounded-xl p-6 text-center">
          <span className="block text-slate-500 text-xs uppercase font-semibold mb-1">AI Rating</span>
          <span className="text-4xl font-extrabold text-emerald-400">{result?.score || '--'} / 100</span>
